...
function check_emulator_status () {
  ...
  while true; do
    result=$(adb -s ${device_name} shell getprop sys.boot_completed 2>&1)

    if [ "$result" == "1" ]; then
      printf "\e[K${G}==> \u2713 Emulator is ready : '$result'           ${NC}\n"
      adb devices -l
      adb -s ${device_name} shell input keyevent 82
      break
    elif [ "$result" == "" ]; then
...
function hidden_policy() {
  adb -s ${device_name} shell "settings put global hidden_api_policy_pre_p_apps 1;settings put global hidden_api_policy_p_apps 1;settings put global hidden_api_policy 1"
};
