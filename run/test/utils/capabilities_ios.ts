 ...
 * speeds up media tests. `FILE_SERVER_PUBKEY` is the file server's **Ed25519** pubkey: LibSession-Util
...
function buildCustomFileServerEnv(): Record<string, string> {
  ...
  if (pubkey && !/^[0-9a-fA-F]{64}$/.test(pubkey)) {
    throw new Error('FILE_SERVER_PUBKEY must be a 64-character hex string (Ed25519 pubkey)');
  }
  return {
