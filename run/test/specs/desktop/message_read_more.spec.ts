 ...
 * `\n` between each line arrives as one run-on line ("line 1line 2line 3..."), which wraps to far fewer
 * rendered lines than it has logical ones.
 *
 * The threshold is rendered LINES (`MAX_MESSAGE_MAX_LINES_BEFORE_READ_MORE`), so this depends on bubble
 * width, which depends on the window. At the harness's window size a bubble fits ~43 characters per
 * line, so 2,000 characters is ~46 lines — over 25 with room to spare. A window roughly twice as wide
 * would put it back on the boundary, which is the first thing to check if the collapse assertion ever
 * fails.
 */
