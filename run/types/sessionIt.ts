// run/types/sessionIt.ts - POC with composite screenshots for 1-2 devices
...
export function registerDevicesForTest(testInfo: TestInfo, devices: DeviceWrapper[], platform: SupportedPlatformsType) {
...
// Add device label overlay to a screenshot
...
async function captureScreenshotsOnFailure(testInfo: TestInfo): Promise<void> {
  const testId = `${testInfo.testId}-${testInfo.repeatEachIndex}`;
  ...
  for (let i = 0; i < context.devices.length; i++) {
    ...
    try {
      ...
      console.error(`Failed to capture screenshot from device ${device.getDeviceIdentity()}:`, error);
    }
  }
  ...
  if (screenshots.length === 0) {
   ...
   try {
    ...
    const attachmentName = screenshots.length === 1 
...
// Simple side-by-side composite for 2 devices
...
// Enhanced test wrapper with screenshot support
type MobileItArgs = {
  platform: SupportedPlatformsType;
